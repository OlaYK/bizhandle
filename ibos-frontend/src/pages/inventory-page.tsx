...
import { z } from "zod";
import { authService, inventoryService, productService } from "../api/services";
...
export function InventoryPage() {
  ...
  const variantsQuery = useQuery({
    ...
    queryFn: () =>
      productService.listVariants(selectedProductId, { limit: 200, offset: 0 }),
  ...
  const selectedProductName = useMemo(() => {
    return (
      productsQuery.data?.items.find((item) => item.id === selectedProductId)
        ?.name ?? "-"
    );
  }, [productsQuery.data, selectedProductId]);
    ...
    <div className="space-y-6">
        ...
        <Card>
          ...
          {!productsQuery.data?.items.length ? (
            <EmptyState
              ...
              description="Create your first product to proceed."
            />
          ) : (
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          ...
          <p className="mt-1 text-sm text-surface-500">
            Current product: {selectedProductName}
          </p>
          <form
            className="mt-4 grid gap-3"
        ...
        <Card>
          ...
          {!variantsQuery.data?.items.length ? (
            <EmptyState
              ...
              description="Create a variant for this product."
            />
          ) : (
            <div className="mt-4 space-y-2">
      ...
      <div className="grid gap-6">
        {/* <Card>
          ...
          <form
            className="mt-4 grid gap-3"
            onSubmit={stockInForm.handleSubmit((values) =>
        ...
        <Card>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Badge variant="negative">
              {lowStockQuery.data?.pagination.total ?? 0}
            </Badge>
          </div>
          {!lowStockQuery.data?.items.length ? (
            <EmptyState
              title="No low stock alerts"
              description="All variants are above threshold."
            />
          ) : (
            <div className="space-y-2">
              {lowStockQuery.data.items.map((item) => (
                ...
                >
                  ...
                  <p className="text-sm text-red-600">
                    {item.size} {item.label ? `- ${item.label}` : ""} | Reorder
              ...
              <PaginationControls
                ...
                onPrev={() =>
                  setLowStockPage((value) => Math.max(1, value - 1))
                }
                onNext={() => {
                ...
                {ledgerQuery.data.items.map((entry) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <p
                        className={`text-sm font-semibold ${entry.qty_delta >= 0 ? "text-mint-700" : "text-red-600"}`}
                      >
                        {entry.qty_delta > 0
                          ? `+${entry.qty_delta}`
                          : entry.qty_delta}
                  ...
                  <tbody className="divide-y divide-surface-50">
                    {ledgerQuery.data.items.map((entry) => (
                      <tr key={entry.id}>
                        ...
                        <td
                          className={`px-2 py-2 font-semibold ${entry.qty_delta >= 0 ? "text-mint-700" : "text-red-600"}`}
                        >
                          {entry.qty_delta > 0
                            ? `+${entry.qty_delta}`
                            : entry.qty_delta}
