...
export function DeveloperPortalPage() {
  ...
  const [apiKeyScopesInput, setApiKeyScopesInput] = useState(
    "business:read,products:read,orders:read,customers:read",
  );
  const [apiKeyExpiresAt, setApiKeyExpiresAt] = useState("");
  const [latestApiKeyValue, setLatestApiKeyValue] = useState<string | null>(
    null,
  );
  ...
  const [subscriptionEvents, setSubscriptionEvents] = useState(
    "storefront.page_view,order.created",
  );
  const [subscriptionDescription, setSubscriptionDescription] = useState("");
  const [latestWebhookSecret, setLatestWebhookSecret] = useState<string | null>(
    null,
  );
  ...
  const [listingScopesInput, setListingScopesInput] = useState(
  ...
  const marketplaceQuery = useQuery({
    ...
    queryFn: () =>
      developerService.listMarketplaceListings({ limit: 50, offset: 0 }),
  });
    ...
    mutationFn: () =>
      developerService.createApiKey({
        ...
        expires_at: apiKeyExpiresAt
          ? new Date(apiKeyExpiresAt).toISOString()
          : undefined,
      }),
    onSuccess: (payload) => {
      ...
      showToast({
        ...
        description:
          "Copy and store the plaintext key now. It is shown only once.",
        variant: "success",
      });
    },
    onError: (error) => {
  ...
  const updateSubscriptionMutation = useMutation({
    mutationFn: (params: {
      ...
      developerService.updateWebhookSubscription(params.subscriptionId, {
  ...
  const rotateWebhookSecretMutation = useMutation({
    mutationFn: (subscriptionId: string) =>
      developerService.rotateWebhookSecret(subscriptionId),
    onSuccess: (payload) => {
      setLatestWebhookSecret(payload.signing_secret);
  ...
  const submitListingMutation = useMutation({
    mutationFn: (listingId: string) =>
      developerService.submitMarketplaceListing(listingId),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["developer", "marketplace-listings"],
      });
  ...
  const publishListingMutation = useMutation({
    mutationFn: (params: { listingId: string; publish: boolean }) =>
      developerService.publishMarketplaceListing(params.listingId, {
  ...
  const probeMutation = useMutation({
    mutationFn: async () => {
      ...
      const products = await publicApiService.products(key, {
        limit: 5,
        offset: 0,
      });
    ...
    onSuccess: (payload) => {
      setProbeResult(
        `Connected to ${payload.profile.business_name}. Products visible: ${payload.productsCount}.`,
      );
    },
    onError: (error) => {
    ...
    <div className="relative space-y-6 after:absolute after:inset-0 after:rounded-2xl after:bg-surface-50 after:cursor-not-allowed after:content-['Coming_Soon'] after:flex after:items-start after:pt-36 after:justify-center after:text-2xl after:font-black after:text-white ">
      <Card className="animate-fade-up bg-[linear-gradient(135deg,#152235_0%,#1f3e61_60%,#2a5f83_100%)] text-white">
        ...
        <p className="mt-1 text-sm text-white/80">
          Manage public API keys, webhook subscriptions, and marketplace listing
          governance from one console.
        </p>
        <div className="mt-3 flex flex-wrap gap-2">
          <Badge variant="info">
            {apiKeysQuery.data.items.length} API keys
          </Badge>
          <Badge variant="info">
            {subscriptionsQuery.data.items.length} webhook subscriptions
          </Badge>
          <Badge variant="info">
            {marketplaceQuery.data.items.length} marketplace listings
          </Badge>
        </div>
      </Card>
      ...
      <Card>
        ...
        <p className="mt-1 text-xs text-surface-500">
          Supported scopes: {supportedScopes || "loading..."}
        </p>
        ...
        {!apiKeysQuery.data.items.length ? (
          <div className="mt-3">
            <EmptyState
              ...
              description="Create your first key for public API access."
            />
          </div>
        ) : (
          <div className="mt-4 space-y-2">
            {apiKeysQuery.data.items.map((item) => (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-surface-500">
                      Prefix:{" "}
                      <span className="font-mono">{item.key_prefix}</span> |
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      loading={
                        ...
                        rotateApiKeyMutation.variables === item.id
                      }
                ...
                <p className="mt-1 text-xs text-surface-500">
                  Updated: {formatDateTime(item.updated_at)} | Last used:{" "}
                  {item.last_used_at
        ...
        {latestWebhookSecret ? (
          <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3">
            <p className="text-xs font-semibold text-emerald-800">
              Latest webhook signing secret
            </p>
            <p className="mt-1 break-all font-mono text-xs text-emerald-900">
              {latestWebhookSecret}
            </p>
          </div>
        ) : null}
        {!subscriptionsQuery.data.items.length ? (
          <div className="mt-3">
            <EmptyState
              ...
              description="Create at least one webhook receiver."
            />
          </div>
        ) : (
          <div className="mt-4 space-y-2">
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge
                      variant={
                        item.status === "active" ? "positive" : "neutral"
                      }
                    >
                      {item.status}
                    </Badge>
                    <Button
                      ...
                      variant="secondary"
                      loading={
                        rotateWebhookSecretMutation.isPending &&
                        rotateWebhookSecretMutation.variables === item.id
                      }
                      onClick={() =>
                        rotateWebhookSecretMutation.mutate(item.id)
                      }
                    >
                      Rotate Secret
                    </Button>
                    <Button
                      ...
                      loading={
                        ...
                        updateSubscriptionMutation.variables?.subscriptionId ===
                          item.id
                      }
                      onClick={() =>
                ...
                <p className="mt-1 text-xs text-surface-500">
                  ...
                  {item.last_delivery_at
                    ? formatDateTime(item.last_delivery_at)
                    : "n/a"}{" "}
                  | Secret hint: {item.secret_hint}
                </p>
              </article>
            ))}
          </div>
        ...
        {!deliveriesQuery.data.items.length ? (
          <div className="mt-3">
            <EmptyState
              ...
              description="Run dispatch after emitting outbox events."
            />
          </div>
        ) : (
          <div className="mt-3 space-y-2">
            {deliveriesQuery.data.items.map((item) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-surface-500">
                  attempts: {item.attempt_count}/{item.max_attempts} | response:{" "}
                  {item.last_response_code ?? "n/a"} | created:{" "}
      ...
      <Card>
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-2">
          <Input
            label="App Key"
            value={listingAppKey}
            onChange={(event) => setListingAppKey(event.target.value)}
          />
          <Input
            label="Display Name"
        ...
        <div className="mt-3">
          <Button
            ...
            disabled={
              ...
              listingDescription.trim().length < 10
            }
        ...
        {!marketplaceQuery.data.items.length ? (
          <div className="mt-3">
            <EmptyState
              ...
              description="Create a draft and move it through review."
            />
          </div>
        ) : (
          <div className="mt-4 space-y-2">
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-surface-500">
                      {item.app_key} | category: {item.category} | scopes:{" "}
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <Button
                    ...
                    loading={
                      ...
                      submitListingMutation.variables === item.id
                    }
      ...
      <Card>
        <h3 className="font-heading text-lg font-bold text-surface-800">
          Developer Docs and SDK Quickstarts
        </h3>
        <div className="mt-3 space-y-2">
